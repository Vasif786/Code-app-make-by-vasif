...
private struct ToolbarView: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            Button(action: {
                App.showTermuxWebView.toggle()
            }) {
                HStack(spacing: 4) {
                    Image(systemName: "globe")
                    Text(App.showTermuxWebView ? "Terminal" : "Termux")
                }
            }
            .help("Toggle Termux WebView")
...
private struct MultiTerminalView: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ZStack {
                if App.showTermuxWebView {
                    EmbeddedBrowser(url: "http://10.103.60.191:7681")
                } else {
